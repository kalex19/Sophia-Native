...
export class CreateAccount extends Component {
  ...
  handleClientSubmit = async () => {
    ...
    if(!username || !password || !password_confirmation || !name || !email || !phone || !address || !state || !city || !zip || !needs || !allergies || !diet || !medications){
			this.setState({message: "Please fill out all input fields"}) 
    } else {
      let user = await postClient(newClientProfile);
      this.props.logIn(user)
			this.setState({message: '', error: user.message})
    }
    if(!error && username && password && password_confirmation && name && email && phone && address && state && city && zip && needs && allergies && diet && medications ){
  ...
  handleCaretakerSubmit = async () => {
    ...
    if(!username || !password || !password_confirmation || !name || !email || !phone || !abilities){
      ...
      let user = await postCaretaker(newCaretakerProfile);
      this.props.logIn(user)
			this.setState({message: '', error: user.message})
    }
      ...
      <View style={styles.routes}>
        ...
        >
          <Text style={styles.registerButton}>
            Register {this.state.accountType === "client" ? "Client" : null}
          </Text>
        </TouchableHighlight>
      </View>
    );
  ...
  render() {
    return (
      <KeyboardAvoidingView style={styles.container} behavior="height" enabled accessible={true}>
          ...
          <Text accessibilityLabel="Scroll to fill out the form inputs below"
              style={styles.text}>Scroll to fill out the form</Text>
          ...
          <Text style={styles.messages} accessibilityLabel={"Please fill out all input fields"}>{this.state.message}</Text>
          <Text style={styles.messages} accessibilityLabel={"Error Message"} >{this.state.error}</Text>
